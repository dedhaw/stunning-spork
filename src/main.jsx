...
export function App() {
  ...
  const captureView = () => {
    ...
    canvas.current.toBlob((blob) => { if (!blob) return setStatus({ message: 'This browser could not capture an image. Try again.', tone: 'error' }); setCaptures((old) => ({ ...old, [view]: blob })); if (view === 'front') { setView('side'); setStatus({ message: 'Front view captured.' }); } else { stream?.getTracks().forEach((track) => track.stop()); setStream(null); setStatus({ message: 'Both views captured.' }); } }, 'image/jpeg', 0.92);
  };
