...
After approving a generated task plan, run `make tasks-run` to open one macOS Terminal session running an independent `codex exec` agent per incomplete, unclaimed task. The launcher is `tasks/scripts/run-task-agents.sh`; terminal sessions start about one second apart by default (`TASK_AGENT_DELAY` can override the delay), while each agent still claims its own task atomically and waits for dependencies. Output is written to `tasks/logs/`; PID files are written to `tasks/runner/`; claims and runtime files are temporary coordination artifacts.
